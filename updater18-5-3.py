        ...
        try:
            ...
            for root, dirs, files in os.walk(self.temp_dir):
                for file in files:
                    ...
                    dst_path = os.path.join(game_folder, rel_path)
    ...
    def revert_patch(self, game_path):
        try:
            ...
            for file_name in files:
                ...
                normalized_path = file_name.replace('/', os.sep)
                file_path = os.path.join(game_folder, normalized_path)
...
class VerifyWorker(ThreadSafeWorker):
    ...
    def run(self):
        try:
            ...
            QMetaObject.invokeMethod(self, "_safe_start_verify", 
    ...
    def _verify_implementation(self):
        ...
        try:
            ...
            files_to_download = 0
            files_to_redownload = 0
            files_deleted = 0
            
            if "downloads" in self.progress_data:
            ...
            if success:
                ...
                self.finished_signal.emit(True, files_to_download, files_to_redownload, files_deleted)
            else:
                self.finished_signal.emit(False, 0, 0, 0)
    ...
    def _verify_files_parallel(self, server_files, total_files):
        ...
        def verify_single_file(file_info):
            ...
            local_path = os.path.join(self.local_dir, normalized_rel_path)
            ...
            with lock:
                if actual_hash != expected_hash:
                    ...
                    if normalized_rel_path not in self.progress_data.get("completed", []):
...
class DownloadWorker(ThreadSafeWorker):
        ...
        try:
            ...
            if 'time=' in result.stdout or 'time<' in result.stdout:
                ...
                if any(x in result.stdout for x in ['time=100', 'time=200']):
                    base_threads = min(12, base_threads + 4)
                return base_threads
        except:
            pass
        return MAX_WORKERS
    ...
    def _download_file(self, f_rel: str, url: str, local_path: str, hash_full: str, idx: int, total_files: int, auth=None):
        ...
        while attempt < self._attempt_limit:
            ...
            try:
                if auth is not None:
                    resp = self.session.get(url, auth=auth, headers=headers, stream=True, timeout=60)
                else:
                ...
                try:
                    ...
                    remaining = 0
                total_size = remaining + downloaded if remaining > 0 else 0
                ...
                with open(local_path, mode) as out:
                    for chunk in resp.iter_content(8192):
                        ...
                        progress_str = f"{overall_percent}% - TF: {idx}/{total_files} DS: {download_speed:.2f}MB/s ETA: {eta_str}"
                        self.current_file_signal.emit(progress_str)

                parts = hash_full.split(":", 1)
                if len(parts) == 2:
    ...
    def run(self):
        try:
            ...
            manifest_url = self.task.get("manifest_url", JSON_SERVER + "manifest.json")
            auth = None
            if self.task.get("use_dav", False):
                auth = HTTPBasicAuth(USERNAME, PASSWORD)
                manifest_url = self.task.get("manifest_url", DAV_SERVER + "manifest.json")

            self._log(f"Загружаем манифест: {JSON_SERVER}manifest.json")
            ...
            if downloads_from_progress:
                ...
                for f_rel in downloads_from_progress.keys():
                    if f_rel in server_files:
                        ...
                        local_path = os.path.join(local_dir, normalize_path(f_rel))
            ...
            def process_file(f_rel, hash_full, local_path, idx):
                ...
                with semaphore:
                    ...
                    if self.task.get("use_dav", False):
                        ...
                        url += f_rel
                    return self._download_file(f_rel, url, local_path, hash_full, idx, total_files, auth=auth)
...
class UpdaterUI(QWidget):
    def _append_log(self, message):
        try:
            print(message)
            if hasattr(self, 'log') and getattr(self, 'log') is not None:
                try:
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Skyrim MO2 Updater + Patcher 18.5.3")
        self.resize(760, 420)
    ...
    def _append_log(self, text: str):
        try:
            if text is None:
                text = ""
            QMetaObject.invokeMethod(self.log, "append", Qt.ConnectionType.QueuedConnection, Q_ARG(str, str(text)))
        except Exception as e:
            ...
            def on_config_finished(config_success, game_path):
                try:
                    ...
                    if config_success:
                        ...
                        self._create_shortcut_after_config()
                    else:
            ...
            def create_shortcut_thread():
                try:
                    ...
                    if success:
                        ...
                        self._append_log("✗ Ошибка создания ярлыка")
                        self._installation_complete(False)
                except Exception as e:
    ...
    def _installation_complete(self, success):
        try:
            ...
            if success:
                self.clear_operation_state()
                self._append_log("=== УСТАНОВКА ЗАВЕРШЕНА УСПЕШНО ===")
                self._append_log("Все шаги выполнены:")
                self._append_log("✓ Проверка папки установки")
                self._append_log("✓ Загрузка файлов")
                self._append_log("✓ Настройка конфигурации INI")
                self._append_log("✓ Создание ярлыка")
            else:
            ...
            self._append_log(f"Ошибка при завершении установки: {e}")
    ...
    def closeEvent(self, event):
        ...
        for thread in threads_to_stop:
            thread.quit()
            if not thread.wait(1000):  # Уменьшили время ожидания
                thread.terminate()
                thread.wait()
